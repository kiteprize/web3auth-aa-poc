import { createPublicClient, createWalletClient, http, parseEther, getContract, encodeFunctionData } from 'viem';
...
if (!networkConfig) {
    throw new Error(`Unsupported network: ${currentNetwork}. Supported: ${Object.keys(NETWORKS).join(", ")}`);
}
...
function loadArtifacts() {
    ...
    const aaArtifactsDir = path.join(__dirname, '..', 'node_modules', '@account-abstraction', 'contracts', 'artifacts');

    const testTokenAbi = JSON.parse(fs.readFileSync(path.join(artifactsDir, 'TestToken.sol', 'TestToken.json'))).abi;
    const myAccountFactoryAbi = JSON.parse(fs.readFileSync(path.join(artifactsDir, 'MyAccountFactory.sol', 'MyAccountFactory.json'))).abi;
    const myAccountAbi = JSON.parse(fs.readFileSync(path.join(artifactsDir, 'MyAccount.sol', 'MyAccount.json'))).abi;
    const entryPointAbi = JSON.parse(fs.readFileSync(path.join(aaArtifactsDir, 'EntryPoint.json'))).abi;

    return { testTokenAbi, myAccountFactoryAbi, myAccountAbi, entryPointAbi };
}

async function createUserOperation(factoryAddress, tokenAddress, accountAddress, userPrivateKey, needsDeployment, entryPointAbi, factoryAbi, tokenAbi, accountAbi) {
    ...
    const accountGasLimits = `0x${verificationGasLimit.toString(16).padStart(32, '0')}${callGasLimit.toString(16).padStart(32, '0')}`;
    ...
    const gasFees = `0x${maxPriorityFeePerGas.toString(16).padStart(32, '0')}${maxFeePerGas.toString(16).padStart(32, '0')}`;
...
async function main() {
    try {
        ...
        const { testTokenAbi, myAccountFactoryAbi, myAccountAbi, entryPointAbi } = loadArtifacts();

        console.log('📋 Using deployed contracts on', currentNetwork, ':');
        console.log('   TestToken:', deploymentData.contracts.TestToken);
        console.log('   MyAccountFactory:', deploymentData.contracts.MyAccountFactory);
        console.log('   EntryPoint:', deploymentData.contracts.EntryPoint);

        // Generate user account
        console.log('\n👤 Generating user account...');
        const userPrivateKey = '0x' + Array(64).fill(0).map(() => Math.floor(Math.random() * 16).toString(16)).join('');
        const userAccount = privateKeyToAccount(userPrivateKey);
        ...
        const smartAccountAddress = await factory.read.getAddress([userAccount.address, BigInt(0)]);
        ...
        const mintHash = await token.write.mint([smartAccountAddress, parseEther('100')], {
            nonce: currentNonce
        });
        ...
        const signedUserOp = await signUserOperation(userOp, userAcc, entryPointAbi);
        ...
        const smartBalanceBefore = await token.read.balanceOf([smartAccountAddress]);
        const devBalanceBefore = await token.read.balanceOf([developerAccount.address]);
        ...
        if (receipt.status === 'success') {
            ...
            const smartBalanceAfter = await token.read.balanceOf([smartAccountAddress]);
            const devBalanceAfter = await token.read.balanceOf([developerAccount.address]);
            ...
            console.log(`💸 Successfully transferred ${transferred} tokens gaslessly!`);
        } else {
